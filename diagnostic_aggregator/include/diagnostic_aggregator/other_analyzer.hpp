...
/*
 *\brief OtherAnalyzer analyzes any messages that haven't been analyzed by other
 *Analyzers
 *
 * OtherAnalyzer is not loaded as a plugin. It is created by the Aggregator, and
 *called seperately. The aggregator will call analyze() on any message not
 *handled by other analyzers.
 *
...
public:
  ...
  {
    ROS_ERROR("OtherAnalyzer was attempted to initialize with a NodeHandle. "
  ...
  /*
   *\brief Reports diagnostics, but doesn't report anything if it doesn't have
   *data
   *
   */
  ...
  {
    std::vector<std::shared_ptr<diagnostic_msgs::msg::DiagnosticStatus>>
    processed = GenericAnalyzerBase::report();

    // We don't report anything if there's no "Other" items
    if (processed.size() == 1) {
      processed.clear();
    } else {
      if (other_as_errors_ && processed.size() > 1) {
        std::vector<std::shared_ptr<diagnostic_msgs::msg::DiagnosticStatus>>::
        iterator it = processed.begin();
