...
class TestAggregator(unittest.TestCase):
    ...
    def test_expected_stale(self):
        ...
        self.sub1 = self.node.create_subscription(
            DiagnosticArray, '/diagnostics', self.diag_cb)
        ...
        with self._mutex:
            if len(self._expecteds) > 0:
                ...
                for name, item in self._expecteds.items():
                    if item.is_stale():
                        #  Stale item in diagnostics, but aggregated didn't report as stale
                        assert(self._agg_expecteds[name].level == 3)
                    else:
                        #  "Diagnostic level of aggregated, raw item don't match for %s" % name)
                        assert(self._agg_expecteds[name].level == item.level)
                self.node.destroy_node()
                self.Test_pass = True
            else:
                print('self._expecteds missing')
